...
def compare_supplier_candidate(
    ...
    exact_item = bool(
        ...
        and str(mf.get("manufacturer_item")).strip()
        == str(sf.get("manufacturer_item")).strip()
    )
    ...
    mf_class = str(
        mf.get("protection_class", "")
    ).strip().upper()
    sf_class = str(
        sf.get("protection_class", "")
    ).strip().upper()

    protection_conflict = bool(
        mf_class and sf_class and mf_class != sf_class
    )

    auto_merge = bool(
        exact_item and not protection_conflict
    )
    ...
    commercial_data_usable = auto_merge
    ...
    return {
        ...
        "commercial_data_usable": commercial_data_usable,
        "pricing_eligible": commercial_data_usable,
        "availability_eligible": commercial_data_usable,
...
def evidence_bundle(
    manufacturer: EvidenceRecord,
    supplier_candidates: list[EvidenceRecord],
) -> dict:
    comparisons = []

    for candidate in supplier_candidates:
        comparison = compare_supplier_candidate(
        ...
        comparisons.append(
            {
                ...
                "commercial_scope": (
                    "USABLE"
                    if comparison["commercial_data_usable"]
                    else "QUARANTINED_NEAR_MATCH"
                ),
            }
        )
    ...
    return {
        ...
        "automatic_supplier_merge_allowed": (
            all(
                x["comparison"]["auto_merge_allowed"]
